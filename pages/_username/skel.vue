<template>
  <div>
    <h1>Event # {{ id }}</h1>
    <v-container>
      <v-row v-for="(item, index) in rowCount" :key="data.id">
        <v-col v-for="(item, index) in numberOfColumns" :key="data.id">
          AA
        </v-col>
      </v-row>
    </v-container>
    <v-container>
      <v-row
        color="blue"
        v-for="(i, ind) in Math.ceil(datum.length / 4)"
        :key="ind"
      >
        <v-col
          color="red"
          lg="3"
          v-for="(item, indx) in datum.slice((i - 1) * 4, i * 4)"
          :key="indx"
        >
          <v-card>
            {{ item.name }}
